    ...
    private void init(final QService service, String resourceName) {
        ...
        addActionListener((ActionEvent e) -> {
            ...
            for (final IWelcome event : ServiceLoader.load(IWelcome.class)) {
                ...
                try {
                    event.buttonPressed(this, service);
                } catch (Throwable tr) {
                ...
                if (service.isLeaf()) {// отсюда и до конца :)
                    ...
                    if (isActive) {
                        ...
                        if (FWelcome.isAdvanceRegim()) {
                            ...
                            for (final IWelcome event : ServiceLoader.load(IWelcome.class)) {
                                QLog.l().logger().info("Вызов SPI расширения. Описание: " + event.getDescription());
                                try {
                                    event.readyNewAdvCustomer(this, res, service);
                                } catch (Throwable tr) {
                                    QLog.l().logger().error("Вызов SPI расширения завершился ошибкой. Описание: " + tr);
                                }
                            }
                        ...
                        if (service.getPreInfoHtml() != null && !"".equals(service.getPreInfoHtml())) {
                            ...
                            for (final IWelcome event : ServiceLoader.load(IWelcome.class)) {
                                ...
                                boolean f = false;
                                try {
                                    f = event.showPreInfoDialog(this, form, FWelcome.netProperty, service.getTextToLocale(QService.Field.PRE_INFO_HTML), service.getTextToLocale(QService.Field.PRE_INFO_PRINT_TEXT), true, true, WelcomeParams.getInstance().delayBack * 2);
                                } catch (Throwable tr) {
                                    QLog.l().logger().error("Вызов SPI расширения завершился ошибкой. Описание: " + tr);
                                }
                                flag = flag && f;
                    ...
                    if (!isActive) {
                        ...
                        if (service.getInput_required()) {
                            ...
                            for (final IWelcome event : ServiceLoader.load(IWelcome.class)) {
                                ...
                                String f = null;
                                try {
                                    f = event.showInputDialog(this, form, true, FWelcome.netProperty, false, WelcomeParams.getInstance().delayBack, service.getTextToLocale(QService.Field.INPUT_CAPTION), service);
                                    cntIn++;
                                } catch (Throwable tr) {
                                    QLog.l().logger().error("Вызов SPI расширения завершился ошибкой. Описание: " + tr);
                                    return;
                    ...
                    if (isActive) {
                        ...
                        for (final IWelcome event : ServiceLoader.load(IWelcome.class)) {
                            ...
                            try {
                                parameters = event.handleStandInParams(this, parameters);
                            } catch (Throwable tr) {
                        ...
                        for (final IWelcome event : ServiceLoader.load(IWelcome.class)) {
                            QLog.l().logger().info("Вызов SPI расширения. Описание: " + event.getDescription());
                            try {
                                event.readyNewCustomer(this, res, service);
                            } catch (Throwable tr) {
                                QLog.l().logger().error("Вызов SPI расширения завершился ошибкой. Описание: " + tr);
                            }
                        }
