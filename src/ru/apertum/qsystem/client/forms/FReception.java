...
public class FReception extends javax.swing.JFrame {
    ...
    private class ServicesMonModel extends AbstractTableModel {
        ...
        public Object getValueAt(int rowIndex, int columnIndex) {
            switch (columnIndex) {
                ...
                case 1:
                    return greed.get(rowIndex).getFirstNumber();
                case 2:
    ...
    private void tableServicesMonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tableServicesMonMouseClicked
        if (evt.getClickCount() > 1 && tableServicesMon.getSelectedRow() != -1) {
            tabsPane.setSelectedComponent(panelServices);
            TreeNode[] nodes = ((DefaultTreeModel) treeServices.getModel()).getPathToRoot(((ATreeModel) treeServices.getModel()).getById(((ServiceInfo) tableServicesMon.getModel().getValueAt(tableServicesMon.getSelectedRow(), 3)).getId()));
            TreePath path = new TreePath(nodes);
            treeServices.setSelectionPath(path);
            treeServices.setExpandsSelectedPaths(true);
            treeServices.scrollPathToVisible(path);
    ...
    public boolean load() {
        ...
        try {
            ...
            tableServicesMon.getColumnModel().getColumn(2).setMaxWidth(200);
            //tableServicesMon.getColumnModel().getColumn(3).setMinWidth(100);
    ...
    public void checkAnyTicket() {
        ...
        if (num != null) {
            ...
            t.getCusts().stream().forEach((c) -> {
                if(c.contains("STATE_FINISH"))
                {
                   c = c.replace("STATE_FINISH", "Завершили обслуживание");
                }
                if(c.contains("STATE_DEAD"))
                {
                   c = c.replace("STATE_DEAD", "Удален из очереди");
                }
                if(c.contains("STATE_WAIT"))
                {
                   c = c.replace("STATE_WAIT", "Ожидает обслуживание");
                }
                if(c.contains("STATE_INVITED"))
                {
                   c = c.replace("STATE_INVITED", "Приглашен");
                }
