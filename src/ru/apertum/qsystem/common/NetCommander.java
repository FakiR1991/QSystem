...
//                QLog.l().logger().trace("Response:\n" + data);
    ...
    public static RpcGetAllServices.ServicesForWelcome getServiсes(INetProperty netProperty) {
        ...
        try {
            ...
            QServiceTree.sailToStorm(rpc.getResult().getRoot(), (TreeNode service) -> {
                ...
                perent.getChildren().forEach(svr -> {
    ...
    public static boolean setBussy(INetProperty netProperty, long userId, boolean lock) {
        ...
        /**
     *Добавление времени логина
     *
     * @param netProperty
     * @param userId id юзера который вызывает
     * @return
     */
    /*public static boolean setLoginTime(INetProperty netProperty, long userId) {
        QLog.l().logger().info("Время логина оператора.");
        final CmdParams params = new CmdParams();
        params.userId = userId;
        // загрузим ответ
        final String res;
        try {
            res = send(netProperty, Uses.TASK_SET_LOGIN_TIME, params);
        } catch (QException ex) {// вывод исключений
            throw new ClientException(Locales.locMes("command_error"), ex);
        }
        final Gson gson = GsonPool.getInstance().borrowGson();
    ...
    */
     ...
     * @return
     */
    public static boolean sendUserStat(INetProperty netProperty, long userId, UsersStatistic st) {
        ...
        final String res;
        try {
            res = send(netProperty, Uses.TASK_SAVE_USER_STAT, params);
        } catch (QException ex) {// вывод исключений
            throw new ClientException(Locales.locMes("command_error"), ex);
        }
        final Gson gson = GsonPool.getInstance().borrowGson();
...
//    }
    ...
    public static void initRoll(INetProperty netProperty, Long serviceId, int first, int last, int current) {
        ...
        final String res;
        try {
            res = send(netProperty, Uses.TASK_REINIT_ROLL, params);
        } catch (QException ex) {// вывод исключений
            throw new ClientException(Locales.locMes("command_error"), ex);
        }
    }
